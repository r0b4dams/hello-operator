...
var _ = Describe("HelloWorld controller", func() {
	Context("HelloWorld controller test", func() {
		...
		AfterEach(func() {
			By("removing the custom resource for the Kind HelloWorld")
			found := &demov1.HelloWorld{}
		...
		It("should successfully reconcile a custom resource for HelloWorld", func() {
			...
			Eventually(func() error {
				if helloworld.Status.Conditions != nil &&
					len(helloworld.Status.Conditions) != 0 {
